class Api::V4::ReceivedFriendRequestsController < ApiController
  ...
  def accept
    if @friend_request.accept
      Pusher.push_to_user(@friend_request.user_id, t('.accepted_notification', name: current_user.name))
      render :show
    else
  ...
  def reject
    if @friend_request.reject
      Pusher.push_to_user(@friend_request.user_id, content: t('.rejected_notification', name: current_user.name))
      render :show
    else
  ...
  def block
    if @friend_request.block
      Pusher.push_to_user(@friend_request.user_id, content: t('.blocked_notification', name: current_user.name))
      render :show
    else

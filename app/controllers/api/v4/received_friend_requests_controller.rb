class Api::V4::ReceivedFriendRequestsController < ApiController
  ...
  def reject
    if @friend_request.reject
      Pusher.push_to_user(@friend_request.user_id, content: t('notification.rejected_friend_request', friend_name: current_user.name))
      render :show
    else
  ...
  def block
    if @friend_request.block
      Pusher.push_to_user(@friend_request.user_id, content: t('notification.blocked_friend_request', friend_name: current_user.name))
      render :show
    else
